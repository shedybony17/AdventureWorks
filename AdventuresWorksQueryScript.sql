...
INSERT INTO Customers(CustomerKey,Prefix,FirstName,LastName,BirthDate,MaritalStatus,Gender,EmailAddress,
AnnualIncome,TotalChildren, EducationLevel,Occupation,HomeOwner)
SELECT CustomerKey, Prefix, FirstName,LastName,BirthDate,MaritalStatus,Gender,EmailAddress,
AnnualIncome,TotalChildren, EducationLevel,Occupation,HomeOwner
...
-----Checking the Total cost of the goods returned by Region

SELECT T.Region, "$" || ROUND(SUM(S.OrderQuantity*P.ProductPrice),2) AS Total_Cost_Of_Goods_Returned
FROM FactSales S
JOIN Products P ON S.ProductKey = P.ProductKey
